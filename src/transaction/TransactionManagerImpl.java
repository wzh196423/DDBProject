...
//                e.printStackTrace();
        ...
        if (preparedList.size() == list.size()) {
            ...
            for (ResourceManager rm : list) {
                try {
                    System.out.println(rm.getID() + "commit xid = " + xid);
                    if (rm.getID().equals(ResourceManager.RMINameFlights)){
                        System.out.println(((Flight)rm.query(xid, rm.getID(), "347")).getNumAvail());
                    }
                    rm.commit(xid);
                } catch (Exception e) {
