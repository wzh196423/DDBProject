 ...
 */
public class ReserveItineraryFailAndQuery {
    ...
    public static void main(String args[]) throws URISyntaxException {
        classpath = new File(ReserveItineraryFailAndQuery.class.getClassLoader().getResource("").toURI()).getPath();
        ...
        try {
            ...
            if (!wc.reserveCar(xid, "John", "SFO")){
                fail("reserveCar fail! xid = " + xid);
            }
            ArrayList<String> flights = new ArrayList<>();
            flights.add("347");
            flights.add("3471");
            // be designed to fail because no car at Stanford
            if (wc.reserveItinerary(xid, "John", flights, "Stanford", true, true)){
                fail("reserveItinerary is designed to fail but success! xid = " + xid );
            }
            if (!wc.commit(xid)){
                fail("Commit " + xid + "fail");
            }
            ...
            if (wc.queryCustomerBill(xid, "John") != 30)
                fail("queryCustomerBill fail. xid = " + xid);

        }
        catch (Exception e) {
            e.printStackTrace();
            fail("ReserveItineraryFailAndQuery fail!");
        }

        System.out.println("ReserveItineraryFailAndQuery pass!");
        dieAll();
