...
public class DieWC {
    ...
    public static void main(String args[]) throws URISyntaxException {
        classpath = new File(DieWC.class.getClassLoader().getResource("").toURI()).getPath();
        ...
        try {
            ...
            if(!wc.addRooms(xid, "SFO", 200, 150)) {
                fail("Add rooms fail!");
            }
            ...
            xid = wc.start();
            if (xid != 2)
                fail("Xid wrong. Expect 2 but = " + xid);
            ArrayList<String> flights = new ArrayList<>();
            flights.add("347");
            if (!wc.reserveItinerary(xid, "John", flights, "SFO",true,true)){
                fail("reserveItinerary fail! xid = " + xid);
            }
            try {
                if (!wc.dieNow(WorkflowController.RMIName)){
                    fail("Die WC failed");
                }
                fail("RemoteException expected! But there's no");
            }catch (RemoteException e){
                System.out.println("Catch exception as expected");
            }

            launch(WorkflowController.RMIName, "WorkflowControllerImpl");

            if (!wc.commit(xid)) {
                fail("Commit failed! xid = " + xid);
            }
